...
class MaskController(object):
    ...
    def update_mask_dimension(self):
        self.mask_model.set_dimension(self.mask_model._img_data.shape)
    ...
    def plot_image(self):
        self.widget.img_widget.plot_image(self.mask_model._img_data, False)
    ...
    def arc_calc_and_preview(self, x, y):
        ...
        arc_center = self.mask_model.find_center_of_circle_from_three_points(v[0], v[1], new_v)
        arc_r = self.mask_model.find_radius_of_circle_from_center_and_point(arc_center, new_v)
        self.arc.arc_center = arc_center
        self.arc.arc_radius = arc_r
        phi_range = self.mask_model.find_n_angles_on_arc_from_three_points_around_p0(arc_center, v[0], v[1], new_v
                                                                                           , 50)
        self.arc.phi_range = phi_range
        arc_points_a = self.mask_model.calc_arc_points_from_angles(arc_center, arc_r, 1, phi_range)
        arc_points_b = self.mask_model.calc_arc_points_from_angles(arc_center, arc_r, -1, phi_range)
    ...
    def arc_width_preview(self, x, y):
        ...
        arc_points_a = self.mask_model.calc_arc_points_from_angles(arc_center, arc_r, -width, phi_range)
        arc_points_b = self.mask_model.calc_arc_points_from_angles(arc_center, arc_r, width, phi_range)
        arc_points = arc_points_a + list(reversed(arc_points_b))
    ...
    def finish_arc(self):
        ...
        self.mask_model.mask_QGraphicsPolygonItem(self.arc)
        self.plot_mask()
        self.widget.img_widget.img_view_box.removeItem(self.arc)
        self.arc = None
    ...
    def below_thresh_btn_click(self):
        ...
        self.mask_model.mask_below_threshold(self.mask_model._img_data, thresh)
        self.plot_mask()

    def above_thresh_btn_click(self):
        thresh = float64(self.widget.above_thresh_txt.text())
        self.mask_model.mask_above_threshold(self.mask_model._img_data, thresh)
        self.plot_mask()
    ...
    def cosmic_btn_click(self):
        self.mask_model.remove_cosmic(self.mask_model._img_data)
        self.plot_mask()
